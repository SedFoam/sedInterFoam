...
(
    ...
    const Foam::volVectorField& Uf,
    const Foam::dragModel& drags
)
:
    phases_(phases),
    Us_(phases.U()),
...
)
{
    ...
    volTensorField dU(fvc::grad(Us_));//gradUst.T(); //that is fvc::grad(Us_);
    volSymmTensorField D(symm(dU));   //0.5*(dU + dU.T)

    volScalarField ThetaClip(Theta_);
    if (limitProduction_) // limit the production by clipping Theta to 2/3kw
    {
        ThetaClip = min(Theta_, (2.0/3.0)*max(kw));
    }

    //////////////////////////////////
    // Radial distribution function g0
    //////////////////////////////////
    gs0_ = radialModel_->g0(min(alphas_, alphasMax_ - alphasSmall), alphasMax_);
    ...
    // and shear stress
    ///////////////////////////////////////
    mus_ = viscosityModel_->mus(alphas_, ThetaClip, gs0_, rhos_, ds_, e_);
    // bulk viscosity  p. 45.
    lambda_ = viscosityModel_->lambda(alphas_, ThetaClip, gs0_, rhos_, ds_, e_);

    // stress tensor, Definitions, Table 3.1, p. 43
    volSymmTensorField tau
    (
        2.0*mus_*D + (lambda_ - (2.0/3.0)*mus_)*tr(D)*I
    );
    //////////////////////
    // Contact dissipation
    //////////////////////
    ...
    volScalarField Lc
    (
        ds_*(alphas_+alphasSmall)/(alphas_+alphasSmall)
    );
    if (extended_)
    {// extended kinetic theory Jenkins (2007)
        ...
        (
            scalar(1),
            0.5*pow(30./(1.+sqr(sqrtPi)/12.)*(1-e_)*sqr(alphas_)*gs0_, 1./3.)
        );
    }
    // Inelastic dissipation (Eq. 3.24, p.50)
    volScalarField gammaCoeff
    (
        (3.0*(1.0 - sqr(e_))*sqr(alphas_)*rhos_*gs0_
    ...
    // NB, drag = K*alphas*alphaw,
    // this is inconsistent with momentum equation,
    // since the following form missed the drift velocity
    /////////////////////////////////////////////////////////
    volScalarField K(drags_.K(mag(Us_ - Uf_)));
    // The following is to calculate parameter tmf_ in u_f*u_s correlation
    // (Danon et al. (1977))
    volScalarField flucVelCor_
    (
        Foam::exp(-B*rhos_*6*epsilonw/max(kw*(1-alphas_)*K, Tpsmall_))
    );
    // Eq. 3.25, p. 50 J_int = J2 - J1*Theta_
    volScalarField J1(alphas_*(1-alphas_)*K*(3+2*quadraticCorrectionJ1_));
    volScalarField J2(alphas_*(1-alphas_)*K*(2*flucVelCor_*kw));
    ...
    (
        1.5*rhos_*phis_*fvc::interpolate((alphas_+alphasSmall))
    );
    ...
    (
       fvm::ddt(1.5*(alphas_+alphasSmall)*rhos_, Theta_)
     + fvm::div(phi, Theta_, "div(phi,Theta)")
     ==
       // Ps term.
       fvm::SuSp(-PsCoeff*fvc::div(phis_), Theta_)
    ...
    PsCoeff = granularPressureModel_->granularPressureCoeff(
          alphas_, gs0_, rhos_, e_);
    ps_ = PsCoeff*Theta_;
    mus_ = viscosityModel_->mus(alphas_, Theta_, gs0_, rhos_, ds_, e_);
    lambda_ = viscosityModel_->lambda(alphas_, Theta_, gs0_, rhos_, ds_, e_);

    ps_.max(0.0);
    ps_.correctBoundaryConditions();

    mus_.max(0.0);
    mus_.correctBoundaryConditions();
