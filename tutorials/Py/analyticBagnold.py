...
def analyticBagnold(nx, H, g, d, rho_p, rho_f, phi0, I0, Bphi, mus, mu2, beta):
    ...
    Ii = I0 / ((mu2 - mus) / (np.tan(beta) - mus) - 1.0)
    alphaex = phi0 / (1 + Bphi * Ii) * np.ones(nx)
    uex = (
        2.0 / 3.0 * Ii * np.sqrt(alphaex * np.cos(beta)) * (H**1.5 - (H - xex) ** (1.5))
    )
    ...
    muIex = (mus + (mu2 - mus) / (I0 / Ii + 1)) * np.ones(nx)
    tauex = muIex * pex
    ...
    return xex, Ii, alphaex, uex, pex, muIex, tauex, duexdz, nuex
